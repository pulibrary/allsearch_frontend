...
describe('ArticleHoldings component', () => {
  test('it shows the journal title if available', () => {
    ...
    expect(wrapper.find('em').text()).toEqual('Journal of Advanced Cats');
  });
  ...
  test('it shows a journal title and full chronology if both are available', () => {
    const wrapper = mount(ArticleCitation, {
      props: {
        fields: {
          publication_title: 'Journal of Advanced Cats',
    ...
    expect(wrapper.text()).toEqual(
      'Journal of Advanced Cats, Vol. 10, No. 6, 2016, pp. 5619-5634'
    );
  });
});
